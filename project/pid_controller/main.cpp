  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          ...
          // steering error is computed from the crosstrack error formula          
          // where p1 point behind car, p2 point ahead of car
          int p1_idx = 0;
          int p2_idx = x_points.size()-1;
          double R_x = x_position - x_points[p1_idx];
          double R_y = y_position - y_points[p1_idx];
          double delta_x = x_points[p2_idx] - x_points[p1_idx];
          double delta_y = y_points[p2_idx] - y_points[p1_idx];
          error_steer = (R_y*delta_x - R_x*delta_y)/(delta_x*delta_x + delta_y*delta_y);
          ...
          **/
          ...
          //error is given by the difference of car actual velocity and target velocity (last point is planner output)
